    ...
    jwt.verify(token, SECRET, async function(err, decoded) {
        if (err) {
            ...
            if (sessionStoreId == null) {
                ...
                // let customerCouponRequestList = await models.customerCouponRequest.findAll({ where: {
                //     [Op.or]: [
                ...
                var customerCouponRequestList = await sequelize.query("SELECT customerCouponRequest.id, customerCouponRequest.customerId, customerCouponRequest.status, customers.fullName, customers.email, customers.mobile FROM `customerCouponRequest` LEFT JOIN `customers` ON customers.id = customerCouponRequest.customerId where customerCouponRequest.storeId = 30",{ type: Sequelize.QueryTypes.SELECT });


                // let listCount = await models.customerCouponRequest.count({where: {
                //     [Op.or]: [
                //         { firstName: { [Op.like]: `%${search}%` } },
                //         { lastName: { [Op.like]: `%${search}%` } },
                //         { email: { [Op.like]: `%${search}%` } },
                ...
                // var listCount = await sequelize.query("SELECT customerCouponRequest.id, customerCouponRequest.customerId, customers.fullName, customers.email, customers.mobile FROM `customerCouponRequest` LEFT JOIN `customers` ON customers.id = customerCouponRequest.customerId where customerCouponRequest.storeId = 30",{ type: Sequelize.QueryTypes.SELECT });

                let pageCount = Math.ceil(customerCouponRequestList.length/pageSize);

                if (customerCouponRequestList) {
                    return res.render('admin/customerCouponRequest/list', {
                        title: 'Customer Coupon Request List',
                        arrData: customerCouponRequestList,
                        storeList: storeList,
                        sessionStoreId: '',
                        listCount: customerCouponRequestList.length,
                        pageCount: pageCount,
                        columnName: column,
                        orderType: order,
                        searchItem: search,
                ...
                if (role == 'admin') {
                    ...
                    userPermission = !!req.session.permissions.find(permission => {
                        return permission === 'customerCouponRequestList'
                    })
                }
                if (userPermission == false) {
                    ...
                    // let customerCouponRequestList = await models.customerCouponRequest.findAll({ order: [[column, order]], where: { storeId: sessionStoreId, [Op.or]: [
                    //     { firstName: { [Op.like]: `%${search}%` } },
                    //     { lastName: { [Op.like]: `%${search}%` } },
                    //     { email: { [Op.like]: `%${search}%` } },
                    //     { contactNo: { [Op.like]: `%${search}%` } },
                    ...
                    // let listCount = await models.customerCouponRequest.count({where: { storeId: sessionStoreId, [Op.or]: [
                    //     { firstName: { [Op.like]: `%${search}%` } },
                    //     { lastName: { [Op.like]: `%${search}%` } },
                    //     { email: { [Op.like]: `%${search}%` } },
                    //     { contactNo: { [Op.like]: `%${search}%` } },
                    ...
                    var customerCouponRequestList = await sequelize.query("SELECT customerCouponRequest.id, customerCouponRequest.customerId, customers.fullName, customers.email, customers.mobile FROM `customerCouponRequest` LEFT JOIN `customers` ON customers.id = customerCouponRequest.customerId where customerCouponRequest.storeId = 30",{ type: Sequelize.QueryTypes.SELECT });
...
exports.delete = function(req, res, next) {
    ...
    jwt.verify(token,SECRET,async function(err,decoded){
        if(err){
            ...
            //     res.redirect('/admin/dashboard');
            // } else {
                await models.customerCouponRequest.destroy({
                    where: { id: id }
                }).then(function (value) {
                    if (value) {
