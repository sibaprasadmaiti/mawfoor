...
exports.addeditProduct = async function(req, res){
  ...
  if(allProductList.length > 0){
    ...
    for(var j=0;j<allProductList.length;j++){
      ...
      if(id !='' && id !=null){
        var relatedProduct = await models.relatedProduct.findAll({attributes:['selectedProductId','id'], where:{productId:id,type:'Related',selectedProductId:allProductList[j].id }});
      ...
      arrRelatedProduct.push({
        "id":allProductList[j].id,
        "productId":allProductList[j].id,
        "title":allProductList[j].title,
        "shortDescription":allProductList[j].shortDescription,
        "slug":allProductList[j].slug,
        "description": allProductList[j].description,
        "price": allProductList[j].price,
        "specialPrice": allProductList[j].specialPrice,
        "weight":allProductList[j].weight,
        "size":allProductList[j].size,
        "status": allProductList[j].status,
        "sequence": allProductList[j].sequence,
  ...
  //For Add-On Products//
  var configProducts = await models.products.findAll({ attributes:['id', 'title', 'storeId','size','price','specialPrice','status'], where:{ isConfigurable:id,storeId:30 } });
  ...
  if(!id){
    return res.render("admin/products/addedit", {
      ...
      arrAllRelatedProductList: arrRelatedProduct,
...
exports.addRelPro = async function(req,res){
  ...
  form.parse(req,async function (err, fields, files) {
    ...
    if(selectProduct !='' && productId !=''){ 
      ...
      models.relatedProduct.destroy({where:{productId: productId,storeId:storeId,}}).then(function(dst){
...
exports.configProduct = async function(req,res){
  ...
  form.parse(req,async function (err, fields, files) {
    ...
    var productDetals = await models.products.findOne({where:{id:org_product_id}});
    var addOneProduct = await models.relatedProduct.findAll({where:{productId : org_product_id,type:"Add-on"}});
    ...
    for(var i=0; i < conf_pro_size.length; i++){
      if(conf_product_id[i] ==''){
        models.products.create({
          ...
          shortDescription : productDetals.short_description,
          ...
          isConfigurable : productDetals.id,
          size : conf_pro_size[i],
          status : con_pro_status[i],
          inventory : productDetals.inventory,
          storeId : productDetals.storeId,
          bestSellers : productDetals.best_sellers,
          newArrivals: productDetals.new_arrivals,
        }).then(function(crt){
          if(addOneProduct.length > 0){
            for(var i=0; i < addOneProduct.length; i++){
              models.relatedProduct.create({
                productId : crt.id,
                selectedProductId : addOneProduct[i].selected_id,
                type: "Add-on"
              })
            }
          }
        ...
        models.products.update({
          ...
          bestSellers : productDetals.best_sellers,
          newArrivals: productDetals.new_arrivals,
        },{where:{id:conf_product_id[i],isConfigurable:org_product_id}}).then(function(upd){
          req.flash("info", "Configurable Product Added Successfully");
          return res.redirect("/admin/products/addedit/"+org_product_id);
        })
      }      
...
exports.deleteProduct = async function(req,res){
  ...
  // }else{
    ...
    // models.products.destroy({where: { id: id, storeId: sessionStoreId }}).then(function(val){
      models.productCategory.destroy({where:{productId: id, storeId: sessionStoreId}}).then(function(val){
      if(val){
          // models.productCategory.destroy({where:{productId: id, storeId: sessionStoreId}}).then(function(dst){
          // models.products.destroy({where: { id: id, storeId: sessionStoreId }}).then(function(dst){
            models.products.destroy({where: { id: id, storeId: sessionStoreId }})
            models.relatedProduct.destroy({where:{productId:id}});
            models.productImages.destroy({where:{productId:id}});
            models.favouriteProduct.destroy({where:{productId:id}});
            models.carts.destroy({where:{productId:id}});
        // })
